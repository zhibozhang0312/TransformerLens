...
class TestActivationCacheCompatibility:
    ...
    def test_cache_key_patterns(self, sample_cache):
        ...
        patterns_found = []
        common_patterns = [
            "embed", "pos_embed", "blocks", "ln_final", "unembed",
            "hook_", "attn", "mlp", "resid"
        ]
    ...
    def test_cache_with_names_filter(self, bridge_model):
        ...
        try:
            ...
            partial_matches = sum(1 for cache_key in cache_keys 
    ...
    def test_cache_batch_dimension_handling(self, bridge_model):
        ...
        prompts = [
            ...
            "Second prompt for batch testing."
        ]
        ...
        try:
            ...
            for key, value in cache_dict.items():
                if value is not None and isinstance(value, torch.Tensor):
                    assert value.shape[0] == len(prompts), f"Tensor {key} has wrong batch size: {value.shape[0]}"
    ...
    def test_cache_memory_efficiency(self, bridge_model):
        ...
        if torch.cuda.is_available():
            ...
            assert memory_growth < 50 * 1024 * 1024, f"Cache caused memory growth of {memory_growth} bytes"
